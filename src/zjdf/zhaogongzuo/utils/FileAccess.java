...
public class FileAccess {
	 ...
	 * @return
	 */
	public static String readBigFile(File f) {
		String text = "";
	 ...
	 * @return 返回字符串
	 */
	public static String readDatasFromAssets(Context context, String name) {
	 ...
	 */
	public static boolean writeJsonIntoFile(String path, String name,
			String content) {
		if (StringUtils.isEmpty(path) || StringUtils.isEmpty(name)
		...
		try {
			out = new OutputStreamWriter(new FileOutputStream(new File(path
					+ name)));
			out.write(content);
			out.flush();
			out.close();
	 ...
	 * @return true:写入成功;false:写入失败
	 */
	public static boolean writeToFile(String path, String name,
			String content) {
		if (StringUtils.isEmpty(path) || StringUtils.isEmpty(name)) {
			return false;
		}
