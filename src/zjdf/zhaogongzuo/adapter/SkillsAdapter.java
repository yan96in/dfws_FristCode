...
import zjdf.zhaogongzuo.activity.resume.AddSkillsActivity;
...
public class SkillsAdapter extends BaseAdapter {
	...
	private List<Skills> listnews;// list数据
	private boolean isManage=false;
	/**选中状态*/
	private boolean[] itemStatus;

	public SkillsAdapter(Context context, List<Skills> lsNews) {
		if (lsNews == null) {
			listnews = new ArrayList<Skills>();
		} else {
			listnews = lsNews;
		}
	...
	// 刷新list
	public void addItems(List<Skills> list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			h = new Holder();
			convertView = layout.inflate(R.layout.layout_my_resume_skills_item,
					null);
			h.linear_context=(LinearLayout)convertView.findViewById(R.id.linear_context);
			h.sk_name = (TextView) convertView.findViewById(R.id.sk_name);
			h.sk_degree = (TextView) convertView.findViewById(R.id.sk_degree);
			h.sk_describe = (TextView) convertView
					.findViewById(R.id.sk_describe);
		...
		final Skills skills = listnews.get(position);
		if (skills!=null) {
			h.sk_name.setText(skills.getSkill_cn());
			h.sk_degree.setText(skills.getAbility_txt());
			h.sk_describe.setText(skills.getDetail_cn());
			//item 跳转到职位
			h.linear_context.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (isManage) {
						...
						String idd=skills.getId();
						if (!StringUtils.isEmpty(idd)) {
							 String name =skills.getSkill_cn(); 
							 String ability=skills.getAbility_txt();
							 String detail=skills.getDetail_cn();
							 Intent intent = new Intent(context,AddSkillsActivity.class);
	...
	private class Holder {
		...
		private TextView sk_name;// 技能名字
		private TextView sk_degree;// 技能掌握程度
		private TextView sk_describe;// 技能描述
