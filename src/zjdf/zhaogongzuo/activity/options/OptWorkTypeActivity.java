 ...
 * @modify 
 */
public class OptWorkTypeActivity extends Activity {
	...
	private void initDatas() {
		// TODO Auto-generated method stub
		if (FrameConfigures.list_work_mode!=null&&FrameConfigures.list_work_mode.size()>0) {
			int nn=FrameConfigures.list_work_mode.size();
			itemViews=new ArrayList<ResumeOptItemView>(nn);
			for (int i=0; i< nn; i++) {
				final OptionKeyValue item=FrameConfigures.list_work_mode.get(i);
				if (item!=null) {
					...
					if (JobFavoriteActivity.desiredJob.work_mode_code!=null&&JobFavoriteActivity.desiredJob.work_mode_code.equals(val)) {
...
//				startResult("不限","0");
		...
		title.setText(R.string.myresume_intention_work_mode);
	}
