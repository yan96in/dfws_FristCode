 ...
 * @modify 
 */
public class OptPositionActivity extends Activity{
	...
	private void initView(){
		...
		title.setText(R.string.myresume_intention_position);
	}
	...
	private View.OnClickListener listener=new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (v==save) {
				if (map_selectedPosition==null||map_selectedPosition.getSize()<1) {
					CustomMessage.showToast(context, "请至少选择一个意向职位！", Gravity.CENTER, 0);
					return;
				}
				formatDatas();
				startIntent();
	...
	private void initSelected(){
		if (JobFavoriteActivity.desiredPositions!=null&&JobFavoriteActivity.desiredPositions.size()>0) {
			for (ResumeIntentionDesiredPosition item : JobFavoriteActivity.desiredPositions) {
				if (item!=null) {
					PositionClassify ar=PositionClassifyController.mapPositionClassifies.get(item.position_code);
					createTextView(ar);
				}
			}
		}
	...
	private void removeTextView(PositionClassify p){
		if (p==null||map_selectedPosition==null||map_selectedViews==null) {
			return;
		}
		final String code=p.getCode();
	...
	private void initDatas(){
		...
		if (PositionClassifyController.positionClassifies!=null&&PositionClassifyController.positionClassifies.size()>0) {
			...
			for (int i = 0; i < n; i++) {
				...
				positionView.setPositionClassify(positionClassify,map_selectedPosition.containKey(positionClassify.getCode())&!positionClassify.isHasSub());
			...
			if (pList != null && pList.size() > 0) {
				...
				for (int i = 0; i < nn; i++) {
					...
					if (p != null) { 
						...
						itemView.setPositionClassify(p,map_selectedPosition.containKey(p.getCode()));
...
//												finish();
	...
	private void formatDatas(){
		if (map_selectedPosition!=null&&map_selectedPosition.getSize()>0) {
			HashMap<String, PositionClassify> map_p=map_selectedPosition.getHashMap();
			int nn=map_p.size();
			List<ResumeIntentionDesiredPosition> positions=new ArrayList<ResumeIntentionDesiredPosition>(nn);
			ResumeIntentionDesiredPosition position=null;
			for (Map.Entry<String, PositionClassify> item : map_p.entrySet()) {
				if (item!=null) {
					position=new ResumeIntentionDesiredPosition();
					position.position_code=item.getKey();
					position.position_value=item.getValue().getValue();
					positions.add(position);
				}
			}
			JobFavoriteActivity.desiredPositions=positions;
		}
	}
