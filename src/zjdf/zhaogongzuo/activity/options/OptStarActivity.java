 ...
 * @modify 
 */
public class OptStarActivity extends Activity {
	...
	private void initDatas() {
		// TODO Auto-generated method stub
		if (FrameConfigures.list_star_level!=null&&FrameConfigures.list_star_level.size()>0) {
			int nn=FrameConfigures.list_star_level.size();
			itemViews=new ArrayList<ResumeOptItemView>(nn);
			for (int i=0; i< nn; i++) {
				final OptionKeyValue item=FrameConfigures.list_star_level.get(i);
				if (item!=null) {
					...
					if (JobFavoriteActivity.star_code.equals(val)) {
...
//				startResult("不限","0");
		...
		title.setText(R.string.myresume_intention_star);
	}
