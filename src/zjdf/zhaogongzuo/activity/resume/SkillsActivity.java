...
import zjdf.zhaogongzuo.adapter.SkillsAdapter;
...
public class SkillsActivity extends BaseActivity {
	...
	private List<Skills> positions;// 数据
	private SkillsAdapter adapter;// 适配器
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.layout_my_resume_skills);
		context = this;
	...
	private void initView() {
		add_skill = getString(R.string.add_skill);
		delete_skill = getString(R.string.delete_skill);
		...
		btn_add_delete.setText(add_skill);
		adapter = new SkillsAdapter(context, positions);
	...
	private void switchBtnLayout() {
		if (isManage) {
			...
			btn_add_delete.setText(delete_skill);
		} else {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				if (positions.size() == 0 || positions == null) {
					Toast.makeText(context, "你还没有添加过技能特长哦", Toast.LENGTH_LONG)
							.show();
					isManage = false;
					switchBtnLayout();
					adapter.clearItems();
	...
	private void getData() {
		...
		executorService.submit(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				positions = myResumeConttroller.get_skills();
	...
	private void doDelete() {
		...
		executorService.submit(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				deleteEorrerBean = myResumeConttroller.delete_skill(idss);
