 ...
 * @modify
 */
public class OptAddressActivity extends Activity {
...
//		if (JobFavoriteActivity.desiredLocations != null
//				&& JobFavoriteActivity.desiredLocations.size() > 0) {
//			for (ResumeIntentionDesiredLocation item : JobFavoriteActivity.desiredLocations) {
//				if (item != null) {
//					Areas ar = AddressController.mapAreas
//							.get(item.location_code);
//					createTextView(ar);
//				}
//			}
//		}
...
//					clearCheckedStatus();
	...
	private void switchSelectedLayout() {
		...
		if (visibility == View.GONE) {
			txt_area_selected_num.setCompoundDrawables(null, null,
					rightDrawable_A, null);
			linear_selected.setVisibility(View.VISIBLE);
		} else {
			txt_area_selected_num.setCompoundDrawables(null, null,
					rightDrawable_V, null);
			linear_selected.setVisibility(View.GONE);
		}
	}
	...
	private void initHotAreas() {
		if (AddressController.hotAreas != null
			...
			for (int i = 0; i < nhot; i++) {
				...
				areaView.setArea(
						area,
						map_selectedArea.containKey(area.getCode())
								& (area.getHassub() != 1));
				...
				for (int i = 0; i < nn; i++) {
					...
					if (a != null) {
						final AddressItemView aItemView = new AddressItemView(
								context);
						aItemView.setArea(a,
								map_selectedArea.containKey(a.getCode()));
						aItemView
