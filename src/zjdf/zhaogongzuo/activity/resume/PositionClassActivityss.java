 ...
 * 
 */
public class PositionClassActivityss extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.layout_search_position_class);
		context=this;
		initView();
		initAnimations();
		initDatas();
		...
		posi_value=getIntent().getStringExtra("posi_value");
	}
	...
	private void initDatas(){
		...
		if (PositionClassifyController.positionClassifies!=null&&PositionClassifyController.positionClassifies.size()>0) {
			...
			for (int i = 0; i < n; i++) {
				...
				positionView.setPositionClassify(positionClassify,posi_code==null?false:(positionClassify.getCode().equals(posi_code)));
			...
			if (pList != null && pList.size() > 0) {
				...
				for (int i = 0; i < nn; i++) {
					...
					if (p != null) { 
						...
						itemView.setPositionClassify(p,posi_code==null?false:(p.getCode().equals(posi_code)));
