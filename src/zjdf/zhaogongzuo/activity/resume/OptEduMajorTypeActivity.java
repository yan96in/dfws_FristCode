 ...
 * @modify
 */
public class OptEduMajorTypeActivity extends Activity {
	...
	private void initDatas() {
		// TODO Auto-generated method stub
		if (FrameConfigures.list_edu_major != null
				&& FrameConfigures.list_edu_major.size() > 0) {
			int nn = FrameConfigures.list_edu_major.size();
			List<OptionKeyValue> ll=FrameConfigures.list_edu_major.subList(1, nn);
			int n=ll.size();
			itemViews = new ArrayList<ResumeOptItemView>(n);
			for (int i = 0; i < n; i++) {
				final OptionKeyValue item = ll.get(i);
				if (item != null) {
