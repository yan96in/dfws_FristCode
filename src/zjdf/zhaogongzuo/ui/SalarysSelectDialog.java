...
import zjdf.zhaogongzuo.activity.personal.OptSalarysActivity;
import zjdf.zhaogongzuo.activity.personal.OptSalarysActivity.IflashViewsCallbacks;
 ...
 * @modify 
 */
public final class SalarysSelectDialog {
	...
	private IflashViewsCallbacks iflashViewsCallback;
	
	public SalarysSelectDialog(Context context) {
	...
	private void initView(final SalaryEnum opt) {
		...
		case SALARY_MODE:
			...
			for (OptionKeyValue entry : FrameConfigures.list_salary_mode) {
				...
				if (key.equals(OptSalarysActivity.mode_value)) {
		...
		case SALARY_CURRENCY:
			...
			for (OptionKeyValue entry : FrameConfigures.list_salary_currency) {
				...
				final Integer value = entry.value;
				itemView.setKeyValue(key, value);
				if (key.equals(OptSalarysActivity.currency_value)) {
					itemView.setChecked(true);
				}else{
					itemView.setChecked(false);
				}
		...
		case SALARY_SCOPE:
			...
			if(OptSalarysActivity.mode_code.equals("1")){
			...
			for (OptionKeyValue entry : list) {
				...
				final Integer value = entry.value;
				itemView.setKeyValue(key, value);
				if (key.equals(OptSalarysActivity.scope_value)) {
					itemView.setChecked(true);
				}else{
					itemView.setChecked(false);
				}
	 ...
	 * @param callback
	 */
	public void setOnDataFlashListener(IflashViewsCallbacks callback){
		iflashViewsCallback=(IflashViewsCallbacks) callback;
	}
